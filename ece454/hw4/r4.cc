...
class sample {
 ...
 public:
  sample *next;
  unsigned count;
  pthread_mutex_t ElementLocks = PTHREAD_MUTEX_INITIALIZER;
  sample(unsigned the_key){my_key = the_key; count = 0;};
  unsigned key(){return my_key;}
  void ElementLock(){pthread_mutex_lock(&ElementLocks);}
  void ElementUnlock(){pthread_mutex_unlock(&ElementLocks);}
...
void *work(void* arg){
  ...
  for (int i=start; i<end; i++){
    ...
    for (int j=0; j<SAMPLES_TO_COLLECT; j++){
      ...
      // Check twice if something alread populated
      // Basically if this is an insert lock the list, 
      // if this is just increment count, increment the count
      if (!(s = h.lookup(key))){
        h.ListLock(key); 
