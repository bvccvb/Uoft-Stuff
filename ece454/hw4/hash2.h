...
template<class Ele, class Keytype> class hash {
 ...
 public:
  ...
  void insert(Ele *e);
  void ListLock(Keytype the_key);
  void ListUnlock(Keytype the_key);
...
hash<Ele,Keytype>::setup(unsigned the_size_log){
  ...
  ListLocks = new pthread_mutex_t[my_size];

  for(int i = 0; i < my_size; i++){
    ListLocks[i] = PTHREAD_MUTEX_INITIALIZER;
  }
}
...
template<class Ele, class Keytype> 
void
hash<Ele,Keytype>::ListLock(Keytype the_key){
  list<Ele,Keytype> *l;

  l = &entries[HASH_INDEX(the_key,my_size_mask)];
  pthread_mutex_lock(&ListLocks[HASH_INDEX(the_key,my_size_mask)]);
  return;
}

template<class Ele, class Keytype> 
void
hash<Ele,Keytype>::ListUnlock(Keytype the_key){
  list<Ele,Keytype> *l;

  l = &entries[HASH_INDEX(the_key,my_size_mask)];
  pthread_mutex_unlock(&ListLocks[HASH_INDEX(the_key,my_size_mask)]);
  return;
} 
