...
public class Bucket implements Runnable
{
	...
	private void updateNoTokens()
	{
		...
		Long elapsed_time = System.nanoTime() - lastTime;
		this.noTokens += elapsed_time / this.tokenInterval;
		if(this.noTokens > size)
			this.noTokens = size;
	...
	public synchronized boolean removeTokens(int noToRemove)
	{
		...
		if(noToRemove > this.noTokens)
			return false;

		this.noTokens -= noToRemove;
		return true;
	}
	...
	public synchronized long getWaitingTime(int tokensToWaitFor)
	{
		...
		return Math.max(0, (tokensToWaitFor - this.noTokens) * this.tokenInterval);
	}
}
