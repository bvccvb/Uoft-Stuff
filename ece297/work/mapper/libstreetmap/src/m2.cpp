...
void draw_directions(){
    ...
    t_bound_box text_boxes;
    
    Directions123 = get_string_vec();
    if (Directions123.size() >0){
         text_boxes = t_bound_box(12 + search_bar_offset, 60 + (50 )+600, width + search_bar_offset - 2, 109 + (50)+600);
     fillrect(text_boxes);
        setcolor(BLACK);

        setfontsize(20);
        drawtext_in(text_boxes, Directions123[selected_direction], FLT_MAX);
    }
        set_coordinate_system(GL_WORLD);
}
...
void key_press(char h, int keysym) {
    ...
    if ((!key_input.empty()) && (h == 8)) {
        ...
        if (Directions123.size() > 0) {
            if (selected_direction > 0)selected_direction--; // -2 is the unselect state
        }
        }else if (keysym == 65363) {

        if (Directions123.size() > 0) {
            if (selected_direction < Directions123.size()-1) selected_direction++; //go down until you hit the end
            }
        } else {
