...
#define ZOOM_FACTOR  1.0667 /* changed from original^^ */
#endif
...
event_loop(void (*act_on_mousebutton)(float x, float y, t_event_buttonPressed button_info),
    void (*act_on_mousemove)(float x, float y),
    void (*act_on_keypress)(char h, int keysym),
    void (*drawscreen) (void)) {
...
draw_message2(void) {
    ...
    if (gl_state.disp_type == SCREEN) {
#ifdef X11
        XClearWindow(x11_state.display, x11_state.external);
        ...
        menutext(
            x11_state.external_draw,
            500,
            50,
            gl_state.statusMessage2
            );
#else
        if (!InvalidateRect(win32_state.hStatusWnd, NULL, TRUE))
            WIN32_DRAW_ERROR();
...
void
update_message2(const string& msg) {
    strncpy(gl_state.statusMessage2, msg.c_str(), BUFSIZE);
    gl_state.statusMessage2[BUFSIZE-1] = '\0'; //Ensure null terimination
    draw_message2();
#ifdef X11
...
void zoom_intersection(double x_left, double x_right, double y_top, double y_bot, void(*drawscreen)(void)) {
    trans_coord.xleft = x_left;
    trans_coord.xright = x_right;
    trans_coord.ytop = y_top;
    trans_coord.ybot = y_bot;
...
translate_up(void (*drawscreen) (void)) {
    ...
    ystep = (trans_coord.ybot - trans_coord.ytop) / 3; // changed; was 2 before
...
translate_left(void (*drawscreen) (void)) {
    ...
    xstep = (trans_coord.xright - trans_coord.xleft) / 3; // changed; was 2 before
...
//    menu_attributes.event_mask = 0;
    ...
    strcpy(button_state.button[10].text, "Change Map");
...
//                        case Button3: /* Right mouse click; pass back to client program */
                        case Button3: /* changed; was Button3 */
...
//                        case Button2: /* Scroll wheel pressed; start panning */
                        case Button2: /* changed; was Button2 */
...
//Added
void x11_build_external (void){
    XSetWindowAttributes menu_attributes2;
    unsigned long valuemask;
    
    
    x11_state.external = XCreateSimpleWindow(x11_state.display, x11_state.toplevel, 0,
        0, 1000,100,
        1, x11_convert_to_xcolor(t_color::predef_colors[BLACK]), 
        x11_convert_to_xcolor(t_color::predef_colors[WHITE]));

    x11_state.external_draw = XftDrawCreate(
        x11_state.display,
        x11_state.external,
        x11_state.visual_info.visual,
        x11_state.colormap_to_use
    ...
    menu_attributes2.event_mask = ExposureMask;
//    menu_attributes.event_mask = 0;
    /* ButtonPresses in this area are ignored. */
    menu_attributes2.do_not_propagate_mask = ButtonPressMask;
    /* Keep text area on bottom left */
    menu_attributes2.win_gravity = SouthWestGravity;
    valuemask = CWWinGravity | CWEventMask | CWDontPropagate;
    XChangeWindowAttributes(x11_state.display, x11_state.external, valuemask, &menu_attributes2);
    XMapWindow(x11_state.display, x11_state.external);
}

void x11_close_external(void){
    XDestroyWindow(x11_state.display, x11_state.external);
}
